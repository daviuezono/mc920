...
int main()
{
    ...
    //le a imagem
    if( ! ler_imagem(&img) ) //erro ao ler imagem?
	{
...
// 	imprimir_imagem(&img);
	...
	liberar_imagem(&img); //libera memoria
	
// 	printf("Imagem Com o Filtro:\n");
...
int ler_imagem(imagem *img)
{
    ...
    {
        if(img_tipo[0]=='P' && (img_tipo[1]=getchar())=='2') //tipo da imagem P2 foi lido?
            break;
        else if(img_tipo[0]=='#') //comentario?
            ler_linha(); //ignora a linha de comentario
    }

    if(img_tipo[0]!='P' || img_tipo[1]!='2') //o tipo da imagem eh P2?
...
imagem *aplicar_filtro(imagem *img)
{
	...
	for (i=0; i<(img->altura); i++)
	{
		...
		{
// 			sup = min(i, i-MASK/2);
// 			inf = max(i, i+MASK/2, img_com_filtro->altura);
// 			esq = min(j, j-MASK/2);
// 			dir = max(j, j+MASK/2, img_com_filtro->largura);
// 			fprintf(stderr, "sup: %d\ninf: %d\nesq: %d\ndir: %d\n", sup, inf, esq, dir);
			
			t = 0;
// 			for (m=sup; m<=inf; m++)
// 				for (n=esq; n<=dir; n++)
// 					vet[t++] = img->pixels[(m-sup)*(dir-esq)+(n-esq)];
			...
			quickSort(vet, 0, t-1);
// 			qsort(vet, t, sizeof(int), compara);
			
			pos = i*(img->largura)+j;
// 			med = (inf-sup)*(dir-esq)/2;
			med = t/2;
			
			fprintf(stderr, "t: %d\n", t);
			
			if (t%2)
				// mediana de um numero par de elementos
				img_com_filtro->pixels[pos] = (vet[med-1] + vet[med])/2;
			else
				// mediana de um numero impar de elementos
